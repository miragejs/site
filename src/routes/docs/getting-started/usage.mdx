...
That's because Mirage's database is empty. We can use the **default scenario** to seed our database with some data:
...
In the example above, we seeded Mirage's database using the `server.create` API in our default scenario:
...
You can use both `server.create` and `server.createList` to invoke your Factories in your default scenario
...
as well as in your testing environment. In a test environment, Mirage will load its routes but ignore the default scenario, giving you the opportunity to set up the database in exactly the state needed for your test:
...
```js
import { Server, hasMany, belongsTo } from "@miragejs/server"
