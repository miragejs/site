...
Now Mirage is running when our app boots up in development! If we write code that fetches data from the network, for example in our App component, we'll see Mirage handle that request with the two users from our `routes()` config:
...
In the `beforeEach` hook we're creating a new server using the same `makeServer` function we defined earlier. We're initializing our server with a `test` environment, which optimizes Mirage from running inside of tests.
...
While this test covers the data loading functionality of our component there's something missing. When we test components like this, we maximize our confidence in them by testing them in various states. When dealing with a data loading component like our App component, we want to be able to answer questions like "What happens when the server has no users?" or "What does our component render if the backend 500s?"
