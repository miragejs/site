...
While Mirage can be used to create one-off servers directly in component or test files, it's best used when your mock server definition is centralized and shared across your development and testing environments. After all, in production your app talks to a single server that implements a single API contract! Using a single server definition thus helps you maintain a consistent mock API contract everywhere Mirage is being used.

So, if already have a Mirage server that you've started for development or testing, move it somewhere where it's clear that it's going to be used in both the development environment and by your tests.

For simpler apps that might look like this:
...
Whereas larger projects might end up with their own `mirage` directory:
...
Next, from your main file (`mirage.js` or `server.js` in the examples above) export a function you can use to start your Mirage server:
