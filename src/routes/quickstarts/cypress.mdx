...
it("shows the users from our server", () => {
  ...
  cy.get('[data-test-id="user-1"]').contains("Luke")
  cy.get('[data-test-id="user-2"]').contains("Leia")
})
...
it("shows a message if there are no users", () => {
  ...
  cy.get('[data-test-id="no-users"]').should("be.visible")
})
```
...
it("handles error responses from the server", () => {
  ...
  cy.get('[data-test-id="server-error"]').contains(
