...
Our app runs, but when it makes a network request to `/api/movies` it errors and the test fails. This is where we can bring in Mirage.
...
When you start up Mirage, it will start intercepting your app's network requests, just like in development. So the next time you run your test, you should see an error like this:
...
We can do that using the `server.create` and `server.createList` methods:
...
## Sharing your Mirage server between development and testing

In the example above, we set up a new server directly within our test, but Mirage is best used when your mock server definition is centralized and shared across your development and testing environments. After all, in production your app will talk to a real server that uses a single API contract! Using a single Mirage server thus helps maintain consistency in your mock API contract.
...
Mirage has an `environment` option that defaults to `development`. In the development environment, Mirage has a default delay of `50ms` to its requests, logs all its responses to the console, and loads the development `seeds`.

Mirage can also be put into a `test` environment, which will set its delays to 0 (unless otherwise overridden) and suppress all of its logging (so as not to dirty up your CI server logs). It will also ignore the `seeds()` function, so that data can be used solely for development but won't leak into or affect your tests. This helps keep your tests deterministic.

To use the `test` environment in our tests, let's update our `startMirage` function to accept an environment option:
...
Now in our tests, we can pass in `test` as the environment, and our tests will run with no delay, we won't get any `seeds()`, and we won't see any logs:
...
If you find yourself debugging a test and want to view the network requests going into and out of Mirage, you can enable logging within that test by setting the `server.logging` option to `true`:
...
Just delete it when you're done to keep your CI testing logs clean.
...
It's also often helpful to use the data from your Mirage server when writing a test, as you can see from the second where Mirage's autogenerated ID is used to visit a dynamic URL for the specific movie.
...
Those are the basics of Application Testing with Mirage! Next let's talk about Integration and Unit tests.
