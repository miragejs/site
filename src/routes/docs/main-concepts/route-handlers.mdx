...
The simplest route handlers are plain functions, and they're the most flexible way to write route handlers in Mirage.

To define new function route handlers, use the `get`, `post`, `patch` (or `put`) and `del` methods. Here's an example:
...
Now when your app makes a GET request to `/movies`, it will receive this data.
