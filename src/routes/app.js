...
//           let Component = cache[`./${route.component}`].default
//
...
function importAll(r) {
  r.keys().forEach(key => {
    ...
    cache[keyWithoutExtension] = r(key)
  })
}
...
export default function(props) {
  let routesService = new RoutesService(props.location.pathname)
      ...
      <ThemeContext.Provider value={theme}>
        ...
        <div className="antialiased text-gray-700 font-body font-light leading-normal min-h-screen flex flex-col">
          ...
          <main className="flex-1 flex flex-col">
            <Router>
              {routesService.flattenedRoutes.map(route => {
                let explicitComponent = cache[`./${route.id}`]
                let NullComponent = () => <></>
