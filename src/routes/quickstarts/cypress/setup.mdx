...
## Step 3: Configure Cypress to define a proxy function for your app's API requests
...
This code defines a `handleFromCypress` function on your application's `window` object. In the next step, we'll configure your app to call this function whenever it makes a network request and Cypress is running.
...
This code is setting up a second Mirage server, but its sole purpose is to act as a proxy for our application's network requests. This is because our real configured Mirage server is going to be running alongside our Cypress code, so that our tests have access to it.

The two Mirage servers are linked through the `handleFromCypress` function, which allows our test code to mock out network requests coming from the application's window.
...
The test environment also disables logging and latency, so that by default your CI test logs will be clean and your tests will be fast. You can set `server.logging = true` if you're writing a test and want to see all the network requests logged to the console.
